...
function ColorSwatch({
  ...
  return (
    <button
      onClick={handleCopy}
      className="group text-left w-full rounded-xl overflow-hidden border border-[var(--fds-color-border)] hover:shadow-[var(--fds-shadow-md)] transition-all"
    ...
    >
      ...
      />
      <div className="px-3 py-2 bg-[var(--fds-color-bg-base)]">
        <p className="text-xs font-semibold text-[var(--fds-color-text-primary)]">{name}</p>
        <p className="text-[11px] font-mono text-[var(--fds-color-text-tertiary)] mt-0.5">
  ...
  return (
    <div className="flex gap-8">
      <article className="flex-1 max-w-3xl px-8 py-10">
        ...
        <h1 className="text-4xl font-bold text-[var(--fds-color-text-primary)] mb-3">색상 (Color)</h1>
        ...
        <section id="primitive">
          <h2 className="text-2xl font-bold text-[var(--fds-color-text-primary)] mb-6">Primitive 토큰</h2>
          ...
          {Object.entries(colorTokens.primitive).map(([colorName, shades]) => (
            <div key={colorName} className="mb-8">
              <h3 className="text-base font-semibold text-[var(--fds-color-text-primary)] capitalize mb-3">
                {colorName}
              </h3>
        ...
        <section id="semantic" className="mt-10">
          <h2 className="text-2xl font-bold text-[var(--fds-color-text-primary)] mb-6">Semantic 토큰</h2>
          <p className="text-[var(--fds-color-text-secondary)] mb-4 text-sm">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[var(--fds-color-bg-elevated)] border-b border-[var(--fds-color-border)]">
                  <th className="text-left px-4 py-3 font-semibold text-[var(--fds-color-text-primary)]">미리보기</th>
                ...
                {Object.entries(colorTokens.semantic).map(([name, token], i, arr) => (
                  <tr key={name} className={i < arr.length - 1 ? 'border-b border-[var(--fds-color-border)]' : ''}>
                    <td className="px-4 py-3">
                      <div
                        className="w-8 h-8 rounded-[var(--fds-radius-md)] border border-[var(--fds-color-border)]"
                        style={{ background: token.value }}
                      />
                    </td>
                    <td className="px-4 py-3 font-mono text-xs text-[var(--fds-color-primary)]">
                      {token.variable}
                    </td>
...
<div style={{ color: 'var(--fds-color-text-secondary)' }}>
  설명 텍스트
</div>`}</code>
            </pre>
          </div>
