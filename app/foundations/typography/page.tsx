  ...
  return (
    <div className="flex gap-8">
      <article className="flex-1 max-w-3xl px-8 py-10">
        ...
        <h1 className="text-4xl font-bold text-[var(--tds-color-text-primary)] mb-3">타이포그래피</h1>
        ...
        <section id="font-family">
          <h2 className="text-2xl font-bold text-[var(--tds-color-text-primary)] mb-4">Font Family</h2>
          <div className="grid grid-cols-1 gap-4 mb-8">
            {[
              ...
              <div key={f.name} className="p-5 rounded-[var(--tds-radius-xl)] border border-[var(--tds-color-border)]">
                <div className="flex items-start justify-between gap-4 mb-3">
                  <div>
                    <p className="text-sm font-semibold text-[var(--tds-color-text-primary)]">{f.name}</p>
                    <p className="text-sm text-[var(--tds-color-text-secondary)]">{f.desc}</p>
                  </div>
                </div>
                <p
                  className="text-2xl text-[var(--tds-color-text-primary)]"
        ...
        <section id="scale">
          <h2 className="text-2xl font-bold text-[var(--tds-color-text-primary)] mb-4">타입 스케일</h2>
          <div className="space-y-1">
            {Object.entries(typographyTokens.scale).reverse().map(([name, token]) => (
              <div
                key={name}
                className="flex items-center gap-4 p-4 rounded-[var(--tds-radius-lg)] hover:bg-[var(--tds-color-bg-elevated)] transition-colors"
              >
                <div className="w-16 shrink-0">
                  <code className="text-xs font-mono text-[var(--tds-color-primary)]">text-{name}</code>
                </div>
                <div className="w-24 shrink-0 text-[var(--tds-color-text-tertiary)] text-xs font-mono">
                  {token.fontSize} / {token.lineHeight}
                </div>
                <div
                  className="flex-1 text-[var(--tds-color-text-primary)] truncate"
        ...
        <section id="weight" className="mt-10">
          <h2 className="text-2xl font-bold text-[var(--tds-color-text-primary)] mb-4">Font Weight</h2>
          <div className="space-y-3">
            {Object.entries(typographyTokens.weight).map(([name, token]) => (
              <div
                key={name}
                className="flex items-center gap-4 p-4 rounded-[var(--tds-radius-lg)] border border-[var(--tds-color-border)]"
              >
                <div className="w-24 shrink-0">
                  <p className="text-xs font-mono text-[var(--tds-color-primary)]">{token.variable}</p>
                  <p className="text-xs text-[var(--tds-color-text-tertiary)] mt-0.5">{token.value}</p>
                </div>
                <p
                  className="text-xl text-[var(--tds-color-text-primary)]"
        ...
        <section id="usage" className="mt-10">
          <h2 className="text-2xl font-bold text-[var(--tds-color-text-primary)] mb-4">폰트 로드</h2>
