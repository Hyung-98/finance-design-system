...
function DemoBox({ label, duration, easing }: { label: string; duration: string; easing: string }) {
  ...
  return (
    <button
      ...
      className="w-full flex flex-col gap-3 p-4 rounded-[var(--tds-radius-lg)] border border-[var(--tds-color-border)] hover:bg-[var(--tds-color-bg-elevated)] text-left"
    >
      <span className="text-xs font-mono text-[var(--tds-color-text-secondary)]">{label}</span>
      <div className="relative h-8 bg-[var(--tds-color-bg-elevated)] rounded-[var(--tds-radius-md)] overflow-hidden">
        <div
          className="absolute top-1 bottom-1 left-1 w-6 rounded-[var(--tds-radius-sm)] bg-[var(--tds-color-primary)]"
          style={{
      ...
      <span className="text-xs text-[var(--tds-color-text-tertiary)]">클릭하여 재생</span>
    </button>
  )
}
  ...
  return (
    <div className="flex gap-8">
      <article className="flex-1 max-w-3xl px-8 py-10">
        ...
        <h1 className="text-4xl font-bold text-[var(--tds-color-text-primary)] mb-3">모션 (Motion)</h1>
        ...
        <section id="duration">
          <h2 className="text-2xl font-bold text-[var(--tds-color-text-primary)] mb-6">Duration (지속시간)</h2>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-[var(--tds-color-bg-elevated)] border-b border-[var(--tds-color-border)]">
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-[var(--tds-color-text-primary)]">사용 상황</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={name} className={i < arr.length - 1 ? 'border-b border-[var(--tds-color-border)]' : ''}>
                    <td className="px-4 py-3">
                      <button
                        ...
                        className="font-mono text-xs text-[var(--tds-color-primary)] hover:underline"
                      >
        ...
        <section id="easing" className="mt-12">
          <h2 className="text-2xl font-bold text-[var(--tds-color-text-primary)] mb-6">Easing (이징)</h2>
          <div className="grid grid-cols-2 gap-4 mb-8">
              ...
              <tbody>
                {Object.entries(motionTokens.easing).map(([name, token], i, arr) => (
                  <tr key={name} className={i < arr.length - 1 ? 'border-b border-[var(--tds-color-border)]' : ''}>
                    ...
                    <td className="px-4 py-3 font-mono text-[11px] text-[var(--tds-color-text-secondary)] break-all">{token.value}</td>
                    <td className="px-4 py-3 text-[var(--tds-color-text-secondary)]">{token.description}</td>
                  </tr>
                ))}
              </tbody>
        ...
        <section id="principles" className="mt-12">
          <h2 className="text-2xl font-bold text-[var(--tds-color-text-primary)] mb-6">모션 원칙</h2>
          <DosDonts
            dos={[
        ...
        <section id="usage" className="mt-10">
          ...
          <div className="rounded-xl overflow-hidden border border-[var(--tds-color-border)]">
            <pre className="p-4 bg-[var(--tds-color-gray-900)] text-sm text-[var(--tds-color-gray-100)] overflow-x-auto">
              <code>{`/* CSS */
.button {
  transition: background-color var(--tds-duration-fast) var(--tds-easing-ease-out);
}

.modal {
  transition: opacity var(--tds-duration-normal) var(--tds-easing-ease-out),
              transform var(--tds-duration-normal) var(--tds-easing-ease-out);
}
